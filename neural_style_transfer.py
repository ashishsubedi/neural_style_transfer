# -*- coding: utf-8 -*-
"""Neural Style Transfer.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1wg2J2yPaUFq1kEN0kiHvwxSpH89THDek
"""

# Commented out IPython magic to ensure Python compatibility.
import PIL
import sys
import IPython.display as display
import time
import numpy as np
import tensorflow as tf
import matplotlib.pyplot as plt
import cv2
from tensorflow.keras.applications.vgg16 import VGG16
import os
import tensorflow.keras.backend as K


n_C = 3
ORIGI_IMAGE_SHAPE = 0
IMAGE_WIDTH, IMAGE_HEIGHT = 800, 800

content_path = ''
style_path = ''
saved_image_path = ''

total_variation_weight = 30

content_layers = ['block4_conv2']
style_layers = ['block1_conv1', 'block2_conv1', 'block3_conv1', 'block5_conv1']
style_weights = [1, 1, 1, 1]
num_content_layers = len(content_layers)
num_style_layers = len(style_layers)
alpha = 1e3
beta = 1e8

epochs = 20
steps_per_epoch = 100

vgg = VGG16(include_top=False, input_shape=(IMAGE_WIDTH, IMAGE_HEIGHT, 3))
vgg.trainable = False


def content_loss(c, g):
    return K.sum(K.square(c-g))


def Gram_Matrix(x):
    features = K.batch_flatten(K.permute_dimensions(x, (2, 0, 1)))
    gram = K.dot(features, K.transpose(features))
    return gram


def style_loss(s, g, w):
    S = Gram_Matrix(s[0])
    C = Gram_Matrix(g[0])
    size = IMAGE_WIDTH * IMAGE_HEIGHT
    return w * K.sum(K.square(S - C)) / (4.0 * (n_C) * (size))


def high_pass_x_y(image):
    x_var = image[:, :, 1:, :] - image[:, :, :-1, :]
    y_var = image[:, 1:, :, :] - image[:, :-1, :, :]

    return x_var, y_var


def total_variation_loss(image):
    x_deltas, y_deltas = high_pass_x_y(image)
    return tf.reduce_sum(tf.abs(x_deltas)) + tf.reduce_sum(tf.abs(y_deltas))


def build_model(vgg, layer_names):
    outputs = [vgg.get_layer(name).output for name in layer_names]
    model = tf.keras.models.Model(vgg.input, outputs)
    return model


def clip_0_1(image):
    return tf.clip_by_value(image, clip_value_min=0.0, clip_value_max=1.0)


model = build_model(vgg, style_layers+content_layers)


def total_loss(outputs):
    style_outputs = outputs[:num_style_layers]
    content_outputs = outputs[num_style_layers:]
    c_loss = K.sum([content_loss(content, content_targets[i])
                    for i, content in enumerate(content_outputs)])
    s_loss = K.sum([style_loss(style, style_targets[i], style_weights[i])
                    for i, style in enumerate(style_outputs)])
    loss = alpha*c_loss + beta * s_loss

    return loss


def load_and_preprocess(path):
	global ORIGI_IMAGE_SHAPE
	img = cv2.imread(path)
	ORIGI_IMAGE_SHAPE = img.shape
	img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = cv2.resize(img, (IMAGE_WIDTH, IMAGE_HEIGHT), cv2.INTER_CUBIC)
    img = img.astype(np.float32)
    img = img/255.0
    img = img.reshape(-1, IMAGE_WIDTH, IMAGE_HEIGHT, n_C)
    img = tf.Variable(img, dtype=np.float32)
    return img

opt = tf.optimizers.Adam(learning_rate=0.02, beta_1=0.99, epsilon=1e-1)


def train_step(img):
        # keep track of our gradients
    with tf.GradientTape() as tape:

        pred = model(img)
        loss = total_loss(pred)
        loss += total_variation_weight*tf.image.total_variation(img)

    # Calculating gradinents of all trainable parameters
    grads = tape.gradient(loss, img)
    # Appling gradient optimization step
    opt.apply_gradients([(grads, img)])
    img.assign(clip_0_1(img))
    loss = np.mean(loss)
    return loss


img = load_and_preprocess(content_path)
style_img = load_and_preprocess(style_path)

style_targets = model(style_img)[:num_style_layers]
content_targets = model(img)[num_style_layers:]

# Run Loop

start = time.time()

step = 0

for n in range(epochs):
    for m in range(steps_per_epoch):
        step += 1
        loss = train_step(img)

end = time.time()

art = img.numpy().squeeze()
art = art * 255.0
art = art.astype(np.uint8)
np.max(art)
art = cv2.cvtColor(art, cv2.COLOR_RGB2BGR)

art = cv2.resize(art, ORIGI_IMAGE_SHAPE))
cv2.imwrite(saved_image_path, art)
